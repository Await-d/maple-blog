 ...
 * Features complete permission management and audit log functionality
 */

import React, { useState } from 'react';
import { Helmet } from '../../components/common/DocumentHead';
import { Shield, Users, FileText, BarChart3, Settings, Zap } from 'lucide-react';
...
import { AccessibilityReport } from '../../components/admin/AccessibilityReport';
...
export const AdminDashboard: React.FC = () => {
  ...
  const auditLogModal = useModal();
  const accessibilityModal = useModal();
  const [activeFeature, setActiveFeature] = useState<'permissions' | 'audit' | 'accessibility' | null>(null);
  
  // Log component mount
  React.useEffect(() => {
    log.info('Admin dashboard mounted', 'componentMount');
    log.logUserAction('page_view', 'admin_dashboard');
  }, [log]);
  return (
    <>
      <Helmet>
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
        <div className="container-responsive py-8">
          <div id="admin-dashboard-content" className="max-w-7xl mx-auto">
            {/* 页面标题 */}
            <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div
                className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-200 cursor-pointer group"
                onClick={() => {
                  setActiveFeature('audit');
                  auditLogModal.openModal();
                }}
              >
                <div className="flex items-center justify-between">
                  <div>
              ...
              <div
                className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-200 cursor-pointer group"
                onClick={() => {
                  setActiveFeature('accessibility');
                  accessibilityModal.openModal();
                }}
              >
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <Zap className="w-8 h-8 text-purple-600 dark:text-purple-400 group-hover:scale-110 transition-transform" />
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                <PermissionManagement className="h-full" />
              </AdminErrorBoundary>
            </Modal>

            {/* Audit Log System Modal */}
            <Modal
              isOpen={auditLogModal.isOpen}
              onClose={() => {
                auditLogModal.closeModal();
                setActiveFeature(null);
              }}
              title="审计日志系统"
              size="full"
            ...
            >
              <AdminErrorBoundary
                ...
                resetOnPropsChange={true}
              >
                <AuditLogSystem className="h-full" />
              </AdminErrorBoundary>
            </Modal>
            ...
            >
              ...
              >
                <AccessibilityReport
                  className="h-full"
                  targetComponentId="admin-dashboard-content"
                  realTime={false}
                />
              </AdminErrorBoundary>
            </Modal>
