...
**重要规则 (CRITICAL RULES):**
- ❌ **禁止模拟数据** - 不允许使用任何模拟数据、临时方案或简化实现
- ✅ **完整功能实现** - 必须对所有功能进行完整的功能实现
...
1. **完整实现** - 任何功能必须完整实现，不允许使用模拟数据或临时解决方案
2. **质量优先** - 所有代码提交前必须通过类型检查、lint 检查和测试
3. **测试管理** - 不创建测试文档，测试完成后自动删除测试文件
4. **架构遵循** - 严格遵循清洁架构分层和现有代码模式
5. **命令执行** - 完成任务后必须运行相应的验证命令
